...
export class BackendAdminService implements IAdminService {
  ...
  async getUsers(): Promise<AdminUser[]> {
    const response = await this.client.get<{ data: AdminUser[] }>('/api/v1/admin/users');
  ...
  async createUser(user: AdminUser & { password?: string }): Promise<void> {
    const response = await this.client.post('/api/v1/admin/users', user);
  ...
  async updateUser(user: AdminUser & { password?: string }): Promise<void> {
    const response = await this.client.put('/api/v1/admin/users', user);
  ...
  async deleteUser(username: string): Promise<void> {
    const response = await this.client.delete(`/api/v1/admin/users/${username}`);
